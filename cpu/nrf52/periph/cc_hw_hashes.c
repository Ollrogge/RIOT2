                                           ...
                                           psa_algorithm_t alg)
{
    ...
    switch(alg) {
        ...
        case PSA_ALG_SHA_1:
            ret = CRYS_HASH_Init(&operation->hw_ctx.cc_ctx, CRYS_HASH_SHA1_mode);
            break;
#endif
#if IS_ACTIVE(CONFIG_CC_HASH_SHA224)
        case PSA_ALG_SHA_224:
            ret = CRYS_HASH_Init(&operation->hw_ctx.cc_ctx, CRYS_HASH_SHA224_mode);
            break;
#endif
#if IS_ACTIVE(CONFIG_CC_HASH_SHA256)
        case PSA_ALG_SHA_256:
            ret = CRYS_HASH_Init(&operation->hw_ctx.cc_ctx, CRYS_HASH_SHA256_mode);
            break;
#endif
        default:
            (void) operation;
                             ...
                             size_t input_length)
{
    ...
    do {
        ...
        switch(operation->alg) {
                ...
                case PSA_ALG_SHA_1:
                    cryptocell_enable();
                    ret = CRYS_HASH_Update(&operation->hw_ctx.cc_ctx, (uint8_t*)(input + offset), size);
                    cryptocell_disable();
                    break;
        #endif
                             ...
                             uint8_t * hash)
{
    ...
    switch(operation->alg) {
                ...
                case PSA_ALG_SHA_1:
                    cryptocell_enable();
                    ret = CRYS_HASH_Finish(&operation->hw_ctx.cc_ctx, (uint32_t*)hash);
                    cryptocell_disable();
                    break;
        #endif
        #if defined(CONFIG_CC_HASH_SHA224)
